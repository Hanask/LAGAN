...
def drawCaption(convas, captions, ixtoword, vis_size, off1=2, off2=2):
    ...
    for i in range(num):
        ...
        for j in range(len(cap)):
            ...
            d.text(((j + off1) * (vis_size + off2), i * FONT_MAX),
                   '%d:%s' % (j, word[:6]),
                   font=fnt,
...
def build_super_images(real_imgs,
                       captions,
                       ixtoword,
                       attn_maps,
                       att_sze,
    ...
    nvis = 4  # 8  ## must less than the numbe of batch size
    ...
    for i in range(num):
        ...
        for j in range(num_attn):
            ...
            if (vis_size // att_sze) > 1:
                # one_map = skimage.transform.pyramid_expand(one_map,
                #                                            sigma=20,
                #                                            upscale=vis_size //
                #                                            att_sze)
                one_map = skimage.transform.pyramid_expand(one_map,
...
def build_super_images2(real_imgs,
    ...
    text_convas = np.ones(
        [batch_size * FONT_MAX, max_word_num * (vis_size + 2), 3],
        dtype=np.uint8)
