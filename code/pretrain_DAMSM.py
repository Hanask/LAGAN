...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=r'/home/rookie/cwt/ControlGAN/code/cfg/DAMSM/sketch_18_4.yml',
        type=str)
...
def evaluate(dataloader, cnn_model, rnn_model, batch_size):
    ...
    w_total_loss = 0
    for step, data in enumerate(dataloader, 0):
        real_imgs, captions, cap_lens, class_ids, keys, wrong_caps, wrong_caps_len, wrong_cls_id = prepare_data(
            data)
...
if __name__ == "__main__":
    ...
    try:
        ...
        for epoch in range(start_epoch, cfg.TRAIN.MAX_EPOCH):
            ...
            count = train(dataloader, image_encoder, text_encoder, batch_size,
                          labels, optimizer, epoch, dataset.ixtoword,
                          image_dir)
            print('-' * 89)
            if len(dataloader_val) > 0:
                ...
                print('| end epoch {:3d} | valid loss '
                      '{:5.2f} {:5.2f} | lr {:.5f}|'.format(
                          epoch, s_loss, w_loss, lr))
            print('-' * 89)
            if lr > cfg.TRAIN.ENCODER_LR / 10.:
                lr *= 0.98

            if (epoch % cfg.TRAIN.SNAPSHOT_INTERVAL == 0
                    or epoch == cfg.TRAIN.MAX_EPOCH):
